...
groupHigh <- read.csv("~/bio321g/DiffExp/R_gdc/high mutated lrpb1/gdc_LUSC_High_mutated_LRP1B.csv",fill=T,header = 1) #edit to change to my gdc files and file locations
groupNon <- read.csv("~/bio321g/DiffExp/R_gdc/non mutated lrpb1/gdc_LUSC_Non_mutated_LRP1B_sequenced.csv", fill=T,header = 1)
...
# Checking query manipulations 
# Checking that groups do not overlap
...
# Prepare the data
# Organizing multidimesional data
dds <- GDCprepare(query = query2,directory = opt$gdcPath)

# Checking if dds is the right class of data
...
# Checking the sample data section
assays(dds) 
...
# Using DESeq2 to create the filter data 
...
#Calculate variance per row in a loop
...
# Calculate scaled PCA with prcomp
...
visuals <- visual[-9, ] #removing row 9 from visual, which is where the outlier is
dds_comp_filter <- dds$comp[-9] #removing the same outlier from dds$comp so that 
...
PCA_plot <- ggplot(
  data = visuals, #data used for ggplot is scaled pca
  mapping = aes(x = PC1, y = PC2, color= dds_comp_filter, shape = dds_comp_filter) #color and shape based off of dds$comp true false values, dds$comp label does appear on legend
)+
  geom_point()+
  theme_bw()+
  coord_fixed()+
...
Significance <- resOutput$padj < 0.01 & abs(resOutput$log2FoldChange) >= 1.5 #subsetting where fdr adjusted p value is <0.01 and log2 fold change is >= 1.5, used abs value to ensure both neg and pos values for fold change are being accounted for 
...
sigplot <- ggplot(
  ...
  ggtitle("Differential Gene Expression Analysis of TGCA-LUSC Tumors
          with No Mutations(50) in LRP1B - High Impact Mutations(31) in LRP1B, 
          p-Value = 0.05")+
  ...
  geom_text_repel(data = head(resOutput[order(resOutput$pvalue),],10), #layer that labels the significant genes taken from the top 10 rows of the ordered p values, different genes labeled if 10 changed to 20
                  ...
                  max.overlaps = 15,
                  size = 3) #considering adding max.overlaps, but volcano plot looks messier, especially with the lines being used to label
...
l2fc <- sum(abs(resOutput$log2FoldChange) > 1.5 & resOutput$padj < 0.01, na.rm = TRUE)

write.csv(resOutput, file = "Differential Expression Results.csv") #making csv file for diff exp results
